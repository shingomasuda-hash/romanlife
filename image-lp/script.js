...
(function () {
  ...
  /* =======================================================================
     4. 画像パネル上のCTA（透明リンク）
        ・日程ボタン → 参加希望日をフォームへ反映し、フォームへスクロール
        ・その他のCTA → フォームへスクロール
     ======================================================================= */
  function initHotspots() {
    $$('[data-pick]').forEach(function (a) {
      a.addEventListener('click', function (ev) {
        ...
        if (d && isClosed(d)) {
          // 締切済みの日程は選べない
          window.alert(d.displayDate + 'の受付は終了しました。別の日程をお選びください。');
  ...
  /* 締切済みの日程は、画像上のボタンにも「受付終了」を重ねて示す */
  function markClosedHotspots() {
    $$('[data-pick]').forEach(function (a) {
      var d = getDate(a.getAttribute('data-pick'));
      if (!d || !isClosed(d)) return;
      a.classList.add('hot-closed');
      a.setAttribute('aria-label', d.displayDate + 'は受付終了しました');
      var tag = document.createElement('span');
      tag.className = 'hot-closed-tag';
      tag.textContent = '受付終了';
      a.appendChild(tag);
    });
  }
  ...
  /* =======================================================================
     7. 計測（Google Analytics 等は dataLayer 経由で後から追加できます）
     ======================================================================= */
